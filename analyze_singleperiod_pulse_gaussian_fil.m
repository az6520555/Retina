...
%         sFR=smooth(FR_cell,50);
        w = gausswin(20);
        FR_gauss=filter(w,1,FR_cell);
        ...
        plot(ax1(channel),SpikeTime_cell,FR_cell);hold on
        plot(ax1(channel),SpikeTime_cell-0.09,FR_gauss);
        lgd=legend(ax1(channel),[' channel',num2str(channel)]) % '9hz',
        lgd.FontSize = 20;
