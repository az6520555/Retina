clearvars -except rate volt lumin_filter offset
...
elseif stimu == 'on'
    ...
    ey(1:at*rate)=aL;
    for i = 1:repeat 
        ey(at*rate+period*rate*(i-1)+1:at*rate+period*rate*(i-1)+rt*rate)= aL; %?e????
...
elseif stimu == 'jk'
    ...
    f=fl;
    t=[];
    beforetest=0.4*I*ones(1,round(beforetime*rate));
    rest=zeros(1,round(restperiod*rate));
    sti=[];
    for i=1:num_pulse
        f=f+df;
        p=1/f;
    	sti=[sti I*ones(1,round(p*duty*rate)) zeros(1,round(p*(1-duty)*rate))];
    end
    ey=[];
    for j=1:repeat-1
...
%% Load mat file
elseif stimu == 'ld'
    filename='HMM_G=5_output';
    load(['\\192.168.1.100\Experiment\Retina\Chou\stimulation code\',filename,'.mat']);
    mean_lumin=30;
    ey=ey-min(ey);
    ey=ey+mean(ey)/5;
    ey=ey*mean_lumin/mean(ey);
    figure(3);plot(t,ey);hold on 
    span=20; %change the smoothing span
    ss=[filename,'_meanLumin=',num2str(mean_lumin),'_'];
%% Gaussian pulse (ON)
elseif stimu == 'gn'
    pw=0.1;
    repeat=40;
    ...
    ttt=1/rate:1/rate:length(ey)/rate;
    plot(ttt,ey);hold on;plot(ttt,a2)
