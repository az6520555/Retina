...
if save_data==1
    ...
    save(n,'Spikes','TimeStamps','a_data','Infos','-v7.3')
end
end
...
%     a_data = diff(a_data);
    a_data2diff=diff(a_data2);
    total_locs=[];
    sectionstep=1000000;
    splitindex=[1:sectionstep:length(a_data2diff) length(a_data2diff)];
    for i=1:length(splitindex)-1
        startendindex=[splitindex(i) splitindex(i+1)];
        [~,locs]=findpeaks(a_data2diff(startendindex(1):startendindex(2)),'MINPEAKHEIGHT',max(a_data2diff)/3);  % the threshold might be change depends on different kind of data
        total_locs=[total_locs,locs+startendindex(1)];
    end
    analog_loc = (total_locs)/SamplingRate;
