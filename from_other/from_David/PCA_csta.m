...
clear all;
close all;
exp_folder = 'F:\§Úªº¶³ºÝµwºÐ\Retina exp\exp data\Sorted_final_data\20200318\';
cd(exp_folder);
save_photo =1;%0 is no save on off photo and data, 1 is save
name = '20200318_cSTA_sort_unit1.mat';%Name that used to save photo and data 'OU_tau=100ms_cutoff=2_08-May-2020_0_sort_unit1.mat'
bin = 5;  BinningInterval = bin*10^-3;  %ms
% %% For unsorted spikes
% load(['data\',name,'.mat'])
%% For sorted spikes
load([exp_folder,name])
...
%% STA
window1 = 1;  %pre spike window
window2 = 1;  %post spike window
