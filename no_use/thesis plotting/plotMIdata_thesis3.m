...
path='F:\我的雲端硬碟\Retina exp\exp data\Sorted_final_data\20190419\MIdata';
cd(path)
...
%     title(['channel ',num2str(channel)])
    for ii=1:2
        ax(ii)=subplot(1,4,[ii*2-1,ii*2]);hold on;box on
...
for i =1:2  
    subplot(subplot(1,4,[i*2-1,i*2]))
    xline(0,'--','color',[0.5,0.5,0.5])
end
legend('\tau=0.1 s','\tau=0.6 s','\tau=1 s','\tau=1.8 s')